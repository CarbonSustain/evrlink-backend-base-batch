...
const validateGiftCardInput = (req, res, next) => {
  const { backgroundId, price, message, giftCardId, recipientAddress } = req.body;
...
// Transfer gift card
router.post('/transfer', verifyToken, validateGiftCardInput, async (req, res) => {
...
router.post('/buy', verifyToken, async (req, res) => {
  try {
    ...
    if (BLOCKCHAIN_ENABLED && req.app.contract) {
      console.log(`🔹 Buying gift card ${giftCardId} on blockchain for ${req.user.walletAddress}`);
      try {
        // Create transaction with payment
        const tx = await req.app.contract.buyGiftCard(giftCardId, { value: price });
        const receipt = await tx.wait();
      ...
      console.log("Blockchain functionality not available or not enabled, proceeding with database update only");
